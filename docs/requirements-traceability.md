...
| Cloud deploy + replication/autoscale/sharding | `infra/terraform`, `infra/bicep`, `docs/cost-strategy.md` |
| Ingest JSON via Kafka | `services/satellite-simulator`, `services/ingestion-service` |
...
| Real-time alerts (threshold breach) | `ingestion-service` (emit) → `alert-service` (persist + WS/SSE) |
...
| Failure events | recorded by `alert-service` on critical; exposed via `GET /failures` |
| IaC (Terraform / Bicep) | `infra/terraform/*.tf`, `infra/bicep/main.bicep` |
| Secrets (Key Vault) | `azurerm_key_vault` + secrets in `infra/terraform/main.tf` |
...
| Documentation (15) | `docs/`, diagrams, this matrix |
